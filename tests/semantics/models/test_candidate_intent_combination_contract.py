...
def test_combine_candidate_intent_records_empty_inputs_returns_empty() -> None:
    result = combine_candidate_intent_records(generated_intents=(), queued_intents=())
    assert result == ()
...
def test_combine_candidate_intent_records_generated_only_origin_is_generated() -> None:
    generated = (_new(client_order_id="n1"), _replace(client_order_id="r1"), _cancel(client_order_id="c1"))
    result = combine_candidate_intent_records(generated_intents=generated, queued_intents=())

    assert tuple(record.intent.client_order_id for record in result) == ("c1", "r1", "n1")
    assert tuple(record.origin for record in result) == (
...
def test_combine_candidate_intent_records_queued_only_origin_is_queued() -> None:
    queued = (_new(client_order_id="n1"), _replace(client_order_id="r1"), _cancel(client_order_id="c1"))
    result = combine_candidate_intent_records(generated_intents=(), queued_intents=queued)
...
def test_combine_candidate_intent_records_keeps_different_keys_with_origins() -> None:
    queued = (_new(client_order_id="order-a"),)
    generated = (_replace(client_order_id="order-b"),)
    result = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)

    assert tuple((r.intent.intent_type, r.intent.client_order_id, r.origin) for r in result) == (
        ("replace", "order-b", CandidateIntentOrigin.GENERATED),
        ("new", "order-a", CandidateIntentOrigin.QUEUED),
    )
...
def test_generated_cancel_dominates_queued_replace_or_new_same_key_origin_generated() -> None:
    key = "same-key"
    queued = (_new(client_order_id=key), _replace(client_order_id=key))
    generated = (_cancel(client_order_id=key),)
    records = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)

    assert len(records) == 1
    assert records[0].intent.intent_type == "cancel"
    assert records[0].origin == CandidateIntentOrigin.GENERATED
...
def test_generated_replace_dominates_queued_new_same_key_origin_generated() -> None:
    key = "same-key"
    queued = (_new(client_order_id=key),)
    generated = (_replace(client_order_id=key),)
    records = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)
    ...
    assert len(records) == 1
    assert records[0].intent.intent_type == "replace"
    assert records[0].origin == CandidateIntentOrigin.GENERATED
...
def test_queued_cancel_dominates_generated_replace_same_key_origin_queued() -> None:
    key = "same-key"
    queued = (_cancel(client_order_id=key),)
    generated = (_replace(client_order_id=key),)
    records = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)
    ...
    assert len(records) == 1
    assert records[0].intent.intent_type == "cancel"
    assert records[0].origin == CandidateIntentOrigin.QUEUED
...
def test_same_type_conflict_latest_wins_origin_follows_winner() -> None:
    key = "same-key"
    queued = (_replace(client_order_id=key, px=101.0),)
    generated = (_replace(client_order_id=key, px=102.0),)
    records = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)
    ...
    assert len(records) == 1
    assert records[0].intent.intent_type == "replace"
    assert records[0].intent.intended_price.value == 102.0
    assert records[0].origin == CandidateIntentOrigin.GENERATED
...
def test_record_output_order_is_priority_then_merge_order_then_key() -> None:
    queued = (
        _new(client_order_id="n-queued"),
        _replace(client_order_id="r-queued"),
        _cancel(client_order_id="c-queued"),
    ...
    assert tuple((record.intent.intent_type, record.intent.client_order_id) for record in records) == (
        ("cancel", "c-queued"),
        ("cancel", "c-generated"),
        ("replace", "r-queued"),
        ("replace", "r-generated"),
    ...
    assert tuple(record.merge_index for record in records) == (2, 3, 1, 4, 0, 5)


def test_combine_candidate_intents_matches_intent_view_of_record_helper() -> None:
    queued = (_new(client_order_id="q-new"), _cancel(client_order_id="q-cancel"))
    generated = (_replace(client_order_id="g-replace"), _new(client_order_id="g-new"))

    compat = combine_candidate_intents(generated_intents=generated, queued_intents=queued)
    records = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)

    assert compat == tuple(record.intent for record in records)


def test_inputs_are_not_mutated() -> None:
    queued = [_new(client_order_id="q1")]
    generated = [_cancel(client_order_id="g1")]
    _ = combine_candidate_intent_records(generated_intents=generated, queued_intents=queued)
